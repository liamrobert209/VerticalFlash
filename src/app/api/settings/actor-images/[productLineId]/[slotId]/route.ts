...
import {
  ...
  clearActorSlotImage,
  actorImagePath,
  ACTOR_IMAGE_SLOT_COUNT,
} from "@/lib/actor-images-store";
...
// filesystem (same reasoning as product-images' equivalent route).
function validate(productLineId: string, slotIdRaw: string): { slotId: number } | NextResponse {
  ...
  if (!Number.isInteger(slotId) || slotId < 1 || slotId > ACTOR_IMAGE_SLOT_COUNT) {
...
export async function GET(
  ...
  const buffer = await fs.readFile(actorImagePath(productLineId, slot.filename)).catch(() => null);
...
export async function POST(
  ...
  try {
    ...
    const slots = await setActorSlotImage(productLineId, check.slotId, file.name, buffer);
...
export async function PATCH(
  ...
  const slots = await setActorSlotLabel(productLineId, check.slotId, label);
  return NextResponse.json({ slots });
}
...
export async function DELETE(
  ...
  const slots = await clearActorSlotImage(productLineId, check.slotId);
  return NextResponse.json({ slots });
}
